...
#@ Boolean(label="Analyze only images with associated ROIs files") ROIs_only
...
#@ Boolean(label="Save objects number summary (by image)") summary
#@ Boolean(label="Skip already analyzed images") skip
...
labels_to_exclude=newArray(0,4,254); // write here the gray level of the labels to exclude separated by comma. Keep the 0,254.
...
//Save 3D suite full features Object tables (M)?
Fluo_M=false
//Save 3D suite full Fluo Object tables (Q)?
Fluo_Q=false
//------------------------------------------------------------------------------------------------------------------------------------

/* This macro analyze object segmentations, as generated by Ilastik object classification and thus including the objects identities (all the objects) and predictions (object types), 
 * Objects can be assigned to ROIs if a .zip ROI file is provided and measure the fluorescence in the orignal image for up to 7 channels.
 ...
 * A subset of the ROIs included in the .zip file can be analyzed by listing them, separated by commas, in files names either areas_to_include.txt or areas_to_excluded.txt inside the ROIs folder.4
 * To use this option it must be activated by changing the Exclude or Include variables to true at row 28 or 30 respectively
 * A subset of the object types (labels) can also be excluded by specifying their index in the options.
 * 
 * For each image the macro require separate files containing objects masks Identities, for individual objects, and predictions (for multiple objects categories), as can be obtainind after Ilastik object classification 
 ...
 * In addition the process nuclei function allow to distinguish completely or incompletely included objects (in the z plane), 
 * uncomment the block at line 442 to activate this option
 * 
 * Results will be saved in the folder specified by the user were multiple sub-folder will be created including the the Assay Name as suffix
 * Results include a table per each image with one row per object specifying different features (mainly volume, coordinates and Fluo values)
 * A summary of the number of objects per ROI per each image can also be produced by activating the "Save objects number summary (by image)" option
...
print("Macro started the "+ year + " " + month +1+ " " + dayOfMonth + " at " +hour + ":" + minute + ":" + second);

setBatchMode(true);

/*
if(labels_to_exclude=="x"){
	labels_to_exclude="";
}
*/
if(ROItag=="x"){
...
//labels_to_exclude=split(labels_to_exclude,",");

number_of_labels=labels_names.length;
// create an array with all possible labels to analyze in 8-bit (0-254)
labels_mgv=Array.getSequence(255);

// remove the labels to exclude from the list of possible mgv values
for(ix=0; ix < labels_to_exclude.length; ix++) {
	labels_mgv=Array.deleteValue(labels_mgv, labels_to_exclude[ix]);
}

print("Analyzed labels:");

for (i = 0; i < number_of_labels; i++) {
	print("		" + labels_mgv[i]+ ": "+labels_names[i]);
}
print("Excluded labels:");
Array.print(labels_to_exclude);
...
Summary_dir= dir +File.separator+Assay_name+"_Summmary_csv";
...
if(summary){
	File.makeDirectory(Summary_dir);
}

Already_Analyzed=newArray();
if(skip){
	Already_Analyzed=getFileList(CSV_Dir);
	print("skipping already analyzed: ");
	for (aa = 0; aa < Already_Analyzed.length; aa++) {
		Already_Analyzed[aa] = replace(Already_Analyzed[aa], "_SegObj_measurements_.csv", "");
		print("			"+Already_Analyzed[aa]);
	}
}
...
for (l = 0; l < InputList.length; l++) {
	...
	//------ PROCESS ONLY IF IMAGES WITH THE SAME BASE NAME HAVE NOT ALREADY BEEN PROCESSED-------------
		...
		if(!contains(fileProcessed,BaseName) & (!contains(Already_Analyzed,BaseName))) {
	...
	//-------CHECK ROIs, OBJECT PREDIICTIONS AND IDENTITIS FILES EXISTENCE----------------------
			...
			getDateAndTime(year, month, dayOfWeek, dayOfMonth, hour, minute, second, msec);
			print("Started at "+ year + " " + month +1+ " " + dayOfMonth + " at " +hour + ":" + minute + ":" + second);
	...
	//--------------IF ALL NECESSARY FILES ARE THERE START PROCESSING -----------------------------
			...
			if(File.exists(Mask_path) && File.exists(Ident_path)&& File.exists(ROI_path) &&FluoImages.length>0){
	...
	//--------------PROCESS ROIs---------------------------	
		...
		// If this option was activated, Delete from ROI manager the areas listed in the areas_to_exclude.txt file that is expected in the ROIs directory
		...
		// If this option was activated, Delete from ROI manager the areas NOT listed in the areas_to_include.txt file that is expected in the ROIs directory
		// the ROI names are expected on a single lines separated by commas
		if (Include) {
		...
		// create an array with the ROIs names, that will be passed to the Process Nuclei Function (as var as it will also pass inside the function)
			var ROI_names=newArray();
	...
	//---------------PROCESS PREDICTIONS AND IDENTITIES----------------------------------------------------------
		...
		//--Createt AN INDENTITY IMAGE FOR EACH LABEL, by fiilling with black all objects belonging to other labels (erasing them supposing a black background). 		
			...
			print("creating label specific identities images and excluding unwanted labels");
			for (i = 0; i < number_of_labels; i++) {
						open(Ident_path);
						rename("Identities_"+labels_mgv[i]);
						run("Properties...", "pixel_width="+pw+" pixel_height="+ph+" voxel_depth="+pd);
						// erase the excluded labels
						for (ex = 0; ex < labels_to_exclude.length; ex++) {
							selectImage(Predictions);
							run("Select Label(s)", "label(s)="+labels_to_exclude[ex]);
							Temp=getImageID();
							// clear the 3D manager and Add labels
							Ext.Manager3D_SelectAll();
							Ext.Manager3D_Delete();
							Ext.Manager3D_AddImage();
							// Erase labels from identity image
							selectWindow("Identities_"+labels_mgv[i]);
		...
		// in example, label 1 of 3 (iteration variable i) will be deleted in Identities_2 and Identities_3 (iteration variable k)	
			for (i = 0; i < number_of_labels; i++) {
					selectImage(Predictions);
					run("Select Label(s)", "label(s)="+labels_mgv[i]); // labels starts from 1
					Temp=getImageID();
					...
					for (k = 0; k < number_of_labels; k++) {
						if(labels_mgv[k] != labels_mgv[i]){
							selectWindow("Identities_"+labels_mgv[k]);
							Ext.Manager3D_Select(0);
							Ext.Manager3D_FillStack(0, 0, 0);
						} else{
							print("found");
	...
	//-----------ANALYZE OBJECTS WITH Process Nuclei Function---------------------		
			...
			for (i = 0; i < number_of_labels; i++) {
					//Inizialize Summary Array
					var Area_summary=newArray();
					var Number_summary=newArray();
				for(k = 0; k < number_of_ROIs; k++){
					Process_Nuclei(labels_mgv[i],k,labels_names[i],ROI_names[k],"all",0);
					}
				...
				for(sm =0; sm < Number_summary.length; sm++){
					...
					saveAs("Results", Summary_dir+File.separator+BaseName+"_summary.csv");
				}
...
getDateAndTime(year, month, dayOfWeek, dayOfMonth, hour, minute, second, msec);
print("Macro ended the "+ year + " " + month +1+ " " + dayOfMonth + " at " +hour + ":" + minute + ":" + second);
...
function Process_Nuclei(LABEL,ROI_index,Nuclei_tag,Area_tag,complete_tag,filling){
...
Area_summary=Array.concat(Area_summary,ROI_names[ROI_index]);
