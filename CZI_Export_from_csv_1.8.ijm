...
//@ Boolean (label="Save multichannel MAX projections", value=true) save_MultiMAX
...
//@ Boolean (label="Save stacks as RGB", value=false) RGB
...
for (line = 0; line < nResults; line++) {
	...
	print("Specimen Folder: "+ SpecimenFolder);
	File.makeDirectory(SpecimenFolder);
	Zdir = SpecimenFolder+File.separator+"splittedMAX";
	mcZdir = SpecimenFolder+File.separator+"MAX";
	
	// Create Folders
	if (save_SplitAndMAX==true) {
		File.makeDirectory(Zdir);
		}	
	if (save_MultiMAX==true) {
		File.makeDirectory(mcZdir);
		}
	if (sequence==true) {
	...
	for (s=0; s<Series_to_export.length-1; s++){
		...
		zValue=z[s];
		imgName=specimen + "_z" + zValue + "_"+ tag;
		rename(imgName);
		...
		if (save_unprocessedstack_as_tiff==true){
	          	...
	          	rename(imgName); // After saving the image is automatically renamed, to keep the original name it must be renamted	 
	          }
		...
		if (save_stack_as_tiff==true){
			if (RGB==true){
				...
				run("RGB Color");
				saveAs("TIF", Stack_Tiff+File.separator+imgName+"_RGB.tif");
				close();
			}
	    ...
	    if (save_MultiMAX==true) {
			...
			saveAs("tif", mcZdir+File.separator+imgName+"_MAX.tif");
	    	rename(imgName);
	    	close();
	   		}
		...
		for(c=0; c<channels; c++){
		      ...
		      if (slices==1){
	             	saveAs("tif", Zdir+File.separator+channel_names[c]+"-"+imgName+".tif");
	         			 close();
	         			 call("java.lang.System.gc");	 
	          } else {
	          //MAX PROJECTIONS
			        run("Z Project...", "projection=[Max Intensity]");
			        saveAs("tif", Zdir+File.separator+channel_names[c]+"-"+imgName+"_MAX.tif");
			  ...
			  //IMAGE SEQUENCE
					 	...
					 	for (j=1; j<=slices; j++){
				 				run("Make Substack..."," slices="+j); 
				  				saveAs("tif", dir2+File.separator+imgName+"_"+IJ.pad(j, 3)+".tif");
