...
// This macro get a selection and generates translated copies of it.
// The direction of the movement is defined by the LeftRight and UpDown radio-buttons
...
for (i = 0; i < num; i++) {
	roiManager("select",nRois-1+i);
	run("Translate... ", "x="+Xdisp+" y="+Ydisp+"");
	roiManager("Add");
	roiManager("select",nRois+i);
	roiManager("rename", name+"-"+(i+1)*disp);	
}
