	...
	//--------------PROCESS ROIs---------------------------	
		...
		// create an array with the area values of each roi that will be saved as txt file
			...
			for (i=0; i<number_of_ROIs;i++){
				...
				Area_value=Array.concat(Area_value,Area);
			}	
		...
		// Save areas as a txt fle
			...
			saveAs("Results", ROIs_Areas+File.separator+BaseName+"_ROI_areas.csv"); 
			run("Clear Results");			
	...
	//-----------ANALYZE OBJECTS WITH Process Nuclei Function---------------------		
			...
			*/
			...
			for(i=0;i<SegObj_labels.length;i++){
				...
				//setResult("CompleteIncomplete", i, split_names[2]);  // activate if distnguish complete and incomplete
