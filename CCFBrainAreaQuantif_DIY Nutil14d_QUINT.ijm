...
#@ String(label="Tag of segmented mask (in the file name)") SegTag
#@ String(value="ATLAS 16-BIT IMAGES", visibility="MESSAGE") hints3
#@ Integer(value="Label gray level: ") tsh
...
print("files in folder: "+list.length);
...
for (k = 0; k < list.length; k++) {
	...
	run("Select None"); // if the image have a selection on a black area, the mean
	getStatistics(area, mean, min, max, std, histogram);
	if (mean>0) {// skip dark images
