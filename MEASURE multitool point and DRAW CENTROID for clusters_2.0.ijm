 ...
 if (endsWith(InputList[l], ".tif")) { 			
	...
	if(File.exists(ROI_path)) {
		...
		for(i=0;i<Cluster_ID.length;i++){
			...
			setResult("ImageName", i, tit);
			setResult("CellType", i, Cells_Type[i]);
			setResult("ID", i, Cluster_ID[i]);
			setResult("X", i, Cluster_X[i]);
			setResult("Y", i, Cluster_Y[i]);
			setResult("Z", i, Cluster_Z[i]);		
		}		
		saveAs("Results", out_dir+File.separator+tit+"_Clusters_Centroid_measurements_.csv");
