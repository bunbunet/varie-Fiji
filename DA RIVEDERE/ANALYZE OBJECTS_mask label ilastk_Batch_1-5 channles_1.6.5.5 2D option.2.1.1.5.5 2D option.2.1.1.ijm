...
#@ Boolean (label="Use a ROIs file?") ROIs_no_ROIs
...
/* This macro analyze objects divided in arbitraty number of categories (lables, object prediction) and regions (ROIs). The output report the classification of each cell 
 * its coordinates and the fluorescence intensity (in up to 5 channels).
