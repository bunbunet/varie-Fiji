...
#@ String(label="Label to exclude (by number, separated by comma, starts from 0, write x to skip)") labels_to_exclude
#@ String(label="Label Names (1 to n) included only") labels_names
...
for (i = 0; i < labels_to_exclude.length; i++) {
	labels_names=Array.deleteIndex(labels_names, labels_to_exclude[i]);
}
