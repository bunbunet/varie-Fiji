...
#@ String(label="Label Names (1 to n) including excluded labels") labels_names
...
for each fluorescence image, image segmentations and rois are retriesved based on the original fluorescence name following the scheme:
	...
	Predictions: Mask-tag + Fluorescence image Name + "_Simple Segmentation.tif"; Labels are exptectetd to start from 1 and increment wthout gaps, as created by Ilastik.
...
Mask Folder
Sp8-P29bP0.5_pz1_c5r2_z130_40Xsp512_merging_1_Simple Segmentation.tif
ROIs folder
...
for (l = 0; l < InputList.length; l++) {
	...
	//---------------PROCESS PREDICTIONS AND IDENTITIES----------------------------------------------------------
		...
		//--CALCULATE THE CONNECTED COMPONENT IMAGE FOR EACH LABEl AS A SEPARATE IMAGE
		...
		/*NOTE that this is not optimal as 16-bit images can be large, thus increasing processing time and limiting the number of labels that can be processed 
		 ...
		 and I'm thus leaving it for future developments. */
...
//loop over objects to append array with SegObj objects main features
