...
#@ String(value="FLUORESCENCE MASKS ROIS AND RESULTS DIRECTORIES", visibility="MESSAGE") hints1
#@ File (label="IMAGE directory", style = "directory") input
#@ File (label="PREDICTIONS and IDENTITIS directory", style = "directory") Mask_dir 
#@ File (label="ROIs directory", style = "directory") ROIs_dir
#@ File (label="Directory to save Results", style = "directory") dir
//#@ String(value="ROIs OPTIONS", visibility="MESSAGE") hints2
...
#@ String(value="TAGS", visibility="MESSAGE") hints3
...
#@ Boolean(label="Save cel number summary (by image)") summary
...
//------------------------ADDITIONAL USER DEFINED OPTIONS:------ C H E C K   I T   O U T------------------------------------------
...
//for summary results a folder for each label will be saved
if(summary){
	for (lb = 0; lb < labels_names.length; lb++) {
		Label_dir=dir +File.separator+Assay_name+"_"+labels_names[lb]+"_summary";
		File.makeDirectory(Label_dir);
	}
}
...
for (l = 0; l < InputList.length; l++) {
	...
	//--------------PROCESS ROIs---------------------------	
		...
		if (Exclude) {
			areas_to_exclude=split(File.openAsString(ROIs_dir+File.separator+"areas_to_exclude.txt"), ",");
		...
		// the ROI names are expected on a single lines separated by commas
		if (Include) {
			areas_to_include=split(File.openAsString(ROIs_dir+File.separator+"areas_to_include.txt"), ",");
			 for (i = roiManager("Count")-1; i >= 0; i--){ 
				roiManager("select", i);
				Region_index=Roi.getName;
	...
	//--------SETS ARRAYS TO STORE RESULTS PER EACH IMAGE---------------------------		
		...
		//Summary
			var Area_summary=newArray();
			var Number_summary=newArray();
	...
	//-----------ANALYZE OBJECTS WITH Process Nuclei Function---------------------		
			...
			for (i = 0; i < number_of_labels; i++) {
				...
				// Save Label number per area Summary
				for(sm =0; sm < Number_summary.length; sm++){
					if(Group_Spec){
						setResult("Group", sm, GroupM);
						setResult("Specimen", sm, Specimen);
						}
						setResult("ROI_name",sm,Area_summary[sm]);
						setResult(labels_names[i]+"_N",sm,Number_summary[sm]);
					}
					saveAs("Results", dir +File.separator+Assay_name+"_"+labels_names[i]+"_summary"+File.separator+BaseName+"_summary.csv");
				}
			...
			*/
			...
			for(i=0;i<SegObj_labels.length;i++){
				...
				setResult("ROI_name", i, split_names[1]);
...
//instead of deleting all the objects, close and re-open
Ext.Manager3D_Close(); 
run("3D Manager");
...
//Ext.Manager3D_SelectAll();
//Ext.Manager3D_Delete(); // Had the impression that this step take so long, I've changed with a close and re-opne the 3D manager
roiManager("Select", ROI);
Ext.Manager3D_AddImage();
Ext.Manager3D_Count(Nb_of_objects);
print("Number of Objects:",Nb_of_objects);

//collect summary results per ROI
Area_summary=Array.concat(Area_summary,ROI);
Number_summary=Array.concat(Number_summary,Nb_of_objects);
